	...
	if (argc < 3) {
		printf("Wrong number of arguments.  Using default values.\n"
			"Usage: lswrto K_PHI K_DISP K_TRIP_DIST\n"
			   "K_PSI       Gain coefficient for how aggressively robot returns to angle.\n"
			   "K_LAT_ERR_THRESH      Gain coefficient for how aggressively robot returns to path.\n"
		);
	} else {
    ROS_INFO("waiting for valid state message...");
		// set control gains for steering
		K_PSI       		= atof(argv[1]);
		K_LAT_ERR_THRESH	= atof(argv[2]);
	}
    // ROS set-ups:
